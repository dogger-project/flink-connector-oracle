...
import io.terminus.fdp.flink.oracle.config.OracleProperties;
import io.terminus.fdp.flink.oracle.connector.ColumnSchema;
import io.terminus.fdp.flink.oracle.connector.OracleTable;
import io.terminus.fdp.flink.oracle.utils.OracleDialect;
...
public class OracleClient implements Serializable {
     ...
     * @throws SQLException SQL异常
     */
    public  int reInsertBySql(String sql, Map<String, Object> maps) throws SQLException {
        /**影响的行数**/
        int affectRowCount = -1;
        PreparedStatement preparedStatement = null;
        try {
     ...
     * @throws SQLException SQL异常
     */
    public  int reUpdate(String sql, Object[] bindArgs) throws SQLException {
        /**影响的行数**/
        int affectRowCount = -1;
        PreparedStatement preparedStatement = null;
        try {
 ...
 /*           if (connection != null) {
    ...
    public static void main(String[] args) throws SQLException {
        OracleProperties op = new OracleProperties();
        op.setPassword("test");
        op.setTableName("sink_table_tmp");
        op.setSchema("test");
        op.setUserName("test");
        op.setUrl("jdbc:oracle:thin:@//127.0.0.1:1521/helowinXDB");
        OracleClient oc = new OracleClient(op);
        System.out.println(oc.tableExists("sink_table_tmp"));
